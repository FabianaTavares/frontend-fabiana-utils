			...
			Object.keys(formObj.controls).forEach(nomeDoControle => {
				...
				if (controle.invalid && !(controle instanceof UntypedFormGroup)) {
					...
					if (controle.parent.parent && controle.parent.parent instanceof UntypedFormArray) {
						...
						seletor =
							option.nomeDoGrupo != null
								? `*[formArrayName=${option.grupoPai}] *[formControlName=${nomeDoControle}]`
								: `*[formControlName=${nomeDoControle}]`;

						elemento =
							(document.querySelector(seletor) as HTMLElement) || (document.querySelector(`*[id=${nomeDoControle}]`) as HTMLElement);
					}
					...
					if (elemento === null) {
						if (!environment.production) {
							console.error(MsgSystemEnum.A006.replace('{VALUE}', nomeDoControle), `[${nomeDoControle}]`);
						}
						return;
					}
					...
					if (nomeElemento === null) {
						if (!environment.production) {
							console.error(MsgSystemEnum.A008.replace('{VALUE}', nomeDoControle), `[${nomeDoControle}]`);
						}
					}

					if (
						controle.parent.parent &&
						controle.parent.parent instanceof UntypedFormArray &&
						...
						controle.parent.parent &&
						controle.parent.parent instanceof UntypedFormArray &&
						controle.hasError('horaInicialMaior')
					) {
						mensagem = MsgSystemEnum.MSG_INATERVALO_HORA_INVALIDO;
					} else if (
						controle.parent.parent &&
						controle.parent.parent instanceof UntypedFormArray &&
						controle.hasError('dataInicialMaior')
					) {
						mensagem = MsgSystemEnum.MSG_INATERVALO_DATA_INVALIDO;
					} else if (
						controle.parent.parent &&
						controle.parent.parent instanceof UntypedFormArray &&
						controle.hasError('dataInicialMenorAtual')
					) {
