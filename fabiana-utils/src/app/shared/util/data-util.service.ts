...
export class DataUtilService {
  ...
  formatDateFormControl(data: Date): Date {
    let dataBr = data.toString().split("-").reverse().join("/")
    return this.convertStringToDate(dataBr);
  }

  formatStringToDataFormControl(dataStr: string): Date {
    let dataBr = dataStr.toString().split("-").reverse().join("/")
    return this.convertStringToDate(dataBr);
  }
  ...
  compararDatas(dataInicio: AbstractControl, dataFinal: AbstractControl):
  boolean {
