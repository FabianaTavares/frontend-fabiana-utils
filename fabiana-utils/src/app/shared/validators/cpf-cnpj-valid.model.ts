  ...
  validaCpf: (cpf) => {
    ...
    if(cpf.length === 11) {
      cpf = cpf.replace(/\D+/g, '');
        ...
        for(let i = 1; i<=9; i++){
          soma = soma + parseInt(cpf.substring(i - 1, i), 0) * (11 - 1);
        }
        resto = (soma * 10) % 11;
  ...
  validaCnpj: (cnpj) => {
    ...
    if(cnpj.length === 14){
      cnpj = cnpj.replace(/\D+/g, '');
        ...
        return (cnpj.charAt(12) === resultadoVerificadorUm.toString() &&
        cnpj.charAt(13) === resultadoVerificadorDois.toString()) ? true : false;
